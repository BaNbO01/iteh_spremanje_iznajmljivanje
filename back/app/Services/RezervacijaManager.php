...
class RezervacijaManager
{
    ...
    public function zakljuciProsle(): array
    {
        $odigrane = $this->rezervacijaService->zakljuciProsle();
        $otkazane = $this->rezervacijaService->otkaziProsleNaCekanju();

        foreach ($otkazane as $rezervacija) {
            $this->obavesti(
                $rezervacija,
                'igrac',
                'Rezervacija je otkazana',
                'Vlasnik terena nije potvrdio rezervaciju pre početka termina, pa je otkazana.'
            );
        }

        return ['odigrane' => $odigrane, 'otkazane' => $otkazane->count()];
    }
