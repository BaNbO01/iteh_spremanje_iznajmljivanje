...
class RezervacijaService
{
    ...
    public function zakljuciProsle(): int
    {
        return Rezervacija::where('status', 'potvrdjena')->prosle()->update(['status' => 'odigrana']);
    }

    public function otkaziProsleNaCekanju(): Collection
    {
        $rezervacije = Rezervacija::where('status', 'na_cekanju')
            ->prosle()
            ->with(['teren.vlasnik', 'igrac'])
            ->get();
        ...
        Rezervacija::whereIn('id', $rezervacije->pluck('id'))->update(['status' => 'otkazana']);

        $rezervacije->each(fn ($rezervacija) => $rezervacija->status = 'otkazana');
